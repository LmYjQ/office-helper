...
namespace piccPdfTool
{
    public class Watermarker
    {
        ...
        private Paragraph CreateVmlWatermark(string text, string color, int fontSize, int angle)
        {
            // VML 形状定义（Word 原生水印格式）
            var shape = new V.Shape()
            {
                Id = "_x0000_s2049",
                Style = BuildVmlStyle(angle),
        ...
        private string BuildVmlStyle(int angle) =>
        ...
        /// <summary>
        /// 同时添加页眉 VML 水印和正文 VML 循环水印（一次打开文件，避免冲突）
        /// </summary>
            ...
            string[]? bodyTexts, int bodyFontSize)
        {
            ...
            using (var doc = WordprocessingDocument.Open(outputPath, true))
            {
                ...
                {
                    var watermarkPara = CreateVmlWatermark(headerText, "#C0C0C0", headerFontSize, -45);
                    var headerPart = mainPart.AddNewPart<HeaderPart>();
                    headerPart.Header = new Header(watermarkPara);
                    string headerId = mainPart.GetIdOfPart(headerPart);
                ...
                {
                    var watermarkPara = CreateVmlWatermarkLoop(bodyTexts, "#C0C0C0", bodyFontSize, -30);
                    var headerPart = mainPart.AddNewPart<HeaderPart>();
                    headerPart.Header = new Header(watermarkPara);
                    string headerId = mainPart.GetIdOfPart(headerPart);
